...
// +kubebuilder:validation:XValidation:rule="!has(self.encapType) || (self.encapType != 'GRE4' && self.encapType != 'L3DSR' && self.encapType != 'NAT4') || !has(self.backends) || self.backends.all(be, be.address.matches('^([0-9]{1,3}[.]){3}[0-9]{1,3}$'))",message="spec.backends addresses must be IPv4 for GRE4/L3DSR/NAT4 encapType"
// +kubebuilder:validation:XValidation:rule="!has(self.encapType) || (self.encapType != 'GRE6' && self.encapType != 'NAT6') || !has(self.backends) || self.backends.all(be, !be.address.matches('^([0-9]{1,3}[.]){3}[0-9]{1,3}$'))",message="spec.backends addresses must be IPv6 for GRE6/NAT6 encapType"
